...
# Return an update matrix A for a given potential energy vector and box width.
function A(V::Vector, L::Real)
    
    ϵ = L / (length(V) - 1)

    AA  = full(SymTridiagonal(0.25(1+ϵ^2*V)im+0.5,-0.125im*ones(length(V)-1)))
    AA[:,1] = AA[:,end] = AA[1,:] = AA[end,:] = zero(typeof(AA[1,1]))
    AA[1,1] = AA[end,end] = 1
    return AA

end

# Convenience methods for A when V is some function and we want N subdivisions
A(V::Function, L::Real, N::Integer, xmin::Real) = A(V((L/N)*[0:N]+xmin),L)
...
function H(V::Vector, L::Real)
    ...
    HH = full(SymTridiagonal((ϵ^-2)-V, -0.5(ϵ^-2)ones(length(V)-1)))
    HH[:,1] = HH[:,end] = HH[1,:] = HH[end,:] = zero(typeof(HH[1,1]))
    HH[1,1] = HH[end,end] = 1
    return HH

end

# Convenience methods for H when V is some function and we want N subdivisions
H(V::Function, L::Real, N::Integer, xmin::Real) = H(V((L/N)*[0:N]+xmin),L)
...
# Convenience method for starting in the center
Φ(k₀::Real, σ::Real) = Φ(k₀, σ, 0)
...
function evolve(A::Matrix, ϕ::Vector, n::Integer, test::Function)
...
evolve(A::Matrix, ϕ::Vector, n::Integer) = evolve(A, ϕ, n, x->true)

# Convenience method for evolving a state vector by passing functions
